...
def resolve_session_path(session_arg):
    ...
    if os.path.isabs(session_arg) or os.path.exists(session_arg):
        return session_arg
    # 否则，强制定位到 ~/.openclaw/super-fetch/ 目录下
...
def main():
    ...
    # 修正点：去掉了原本行末多出的反引号 `
    parser.add_argument("--session", "-s", default="session.json", help="Session 文件名")
    ...
    # 路径解析：确保默认的 session.json 也能正确落入 DATA_DIR
    session_path = resolve_session_path(args.session)

    # 交互模式逻辑锚点：-i 自动强制开启 playwright
    if args.interactive:
